...
class STPN(nn.Module):
    def __init__(self, height_feat_size):
        ...
        # self.conv3d_1 = Conv3D(64, 64, kernel_size=(3, 1, 1), stride=1, padding=(0, 0, 0))  # input temporal shape 5
        self.conv3d_1 = Conv3D(64, 64, kernel_size=(1, 1, 1), stride=1, padding=(0, 0, 0))  # input temporal shape 3
...
if __name__=='__main__':
    model = STPN(height_feat_size=32)
    with torch.no_grad():
        input = torch.randn((2, 32, 3, 256, 256))
        output = model(input)
        print(output.shape)